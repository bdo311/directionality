...
if '.pkl' in sys.argv[1]:
    (pos_to_dhs, dhsnum_to_dhs) = pickle.load(open(sys.argv[1], 'r'))
else:
    ...
    for row in reader:
        ...
        pos_to_dhs[row[0]][int(row[1])/100] = dhsnum
        
    ifile.close()
    pickle.dump((pos_to_dhs, dhsnum_to_dhs), open(sys.argv[1][:-4] + '.pkl', 'w'))
...
ifile.close()
pickle.dump((dict(dhs_to_dhs), dhsnum_to_dhs, total_good, twentyctr, ctr), open(sys.argv[3], 'w'))
